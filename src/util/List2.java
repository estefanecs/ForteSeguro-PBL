...
public class List2 {
    ...
    public Vertice procurarNo(String m) {
        ...
        while (auxiliar != null) {
            if (auxiliar.getConteudo().getNome().compareToIgnoreCase(m)==0) {
    ...
    public int getPosicao(String n) {
        ...
        int posicao = 0;
        while (aux != null) {
            if (aux.getConteudo().getNome().compareToIgnoreCase(n)==0) {
                return posicao;
            }
            aux = aux.getNext();
            posicao++;
    ...
    public Vertice remove(String nome) {
        Node aux = primeiro;
        Node aux2 = primeiro;
        if (primeiro.getConteudo().getNome().compareToIgnoreCase(nome)==0) {
     ...
     */
    public Iterator procurarPonto(String TipoPonto) {
        List2 listaAuxiliar = new List2(); //Lista auxiliar
        ...
        while (iterar.hasNext()) {
            ...
            if (percorrer.getTipo().compareToIgnoreCase(TipoPonto)==0) { //Se o procedimento for igual
